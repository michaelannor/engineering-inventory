...
-->
				...
				<td id="content">
					...
					<div id="divContent">
					<form action="search_GpManu.php" method="get">
...
if(isset($_REQUEST['st'])){
	...
	if(!$obj->searchManu($search_text)){
		echo "Error searching";
	}
	echo "<table border='1'>";
echo"<tr><td>Manufacturer_ID</td><td>Manufacturer_Name</td><td>Delete</td>
<td>Edit</td></tr>";
$row=$obj->fetch();
while($row){
	echo "<tr><td> ".$row['Manufacturer_ID']."</td><td><a href=View_GpManu.php?id=".$row['Manufacturer_ID'].">
	".$row['Manufacturer_Name']."</a></td><td><a href=delete_GpManu.php?id=".$row['Manufacturer_ID'].">delete</a>
	</td><td><a href=update_GpManu.php?id=".$row['Manufacturer_ID'].">edit</a> </td></tr>";
    $row=$obj->fetch();
}
